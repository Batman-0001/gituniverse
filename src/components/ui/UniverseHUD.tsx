...
const stellarTypeConfig: Record<
  string,
...
export default function UniverseHUD() {
  ...
  const viewLevelLabel =
    viewLevel === "solar-system" ? "Solar System" : (
    ...
    >
      ...
      <motion.div
        ...
        style={{
          ...
          background:
            "linear-gradient(180deg, rgba(3,0,20,0.85) 0%, transparent 100%)",
          pointerEvents: "auto",
        }}
      >
        {/* Left: Back + Repo name */}
        ...
        >
          {[
            ...
            {
              icon: GitBranch,
              value: repository.totalCommits,
              label: "Commits",
              color: "var(--blue-giant)",
            },
            {
              icon: Layers,
              value: stats.totalGalaxies,
              label: "Galaxies",
              color: "var(--galaxy-teal)",
            },
            {
              icon: Orbit,
              value: stats.binaryStars,
              label: "Binary",
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                justifyContent: "center",
                color:
                  ctrl.active ? "var(--blue-giant)" : "rgba(255,255,255,0.3)",
                cursor: "pointer",
              }}
            >
              <ctrl.icon size={14} />
          ...
          {sidebarOpen && (
            ...
            >
              ...
              {spatialData?.developers
                ...
                .map((dev) => {
                  const config =
                    stellarTypeConfig[dev.stellarType] ||
                    stellarTypeConfig.yellow_sun;
                  ...
                  return (
                    <motion.button
                      ...
                      style={{
                        ...
                        borderRadius: "var(--radius-sm)",
                        background:
                          isActive ? "rgba(79,195,247,0.12)" : "transparent",
                        border:
                          isActive ?
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        >
                          {dev.totalCommits} commits · mass{" "}
                          {dev.normalizedMass.toFixed(0)}
                        </div>
                      </div>
                      {!dev.isActive && (
                        <Star
                          size={10}
                          style={{ color: "var(--white-dwarf)", flexShrink: 0 }}
                        />
                      )}
                    </motion.button>
                  );
      ...
      <AnimatePresence>
        {selectedDev && <DeveloperPanel developer={selectedDev} />}
      </AnimatePresence>

      {/* ── Galaxy Detail Panel (Right) ───────────────────── */}
      <AnimatePresence>
        {selectedGalaxyId && !selectedDev && <GalaxyPanel />}
      </AnimatePresence>
